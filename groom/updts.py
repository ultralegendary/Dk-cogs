...
class Updts(commands.Cog):
    def __init__(self, bot):
        ...
        self.config.register_guild(channel_id=None,guild_id=None,)
    ...
    async def update(self, ctx):
        ...
        async with ctx.typing():
            ...
            teachers = []
            for c in courses[:11]:
                tech =                     self.service.courses()                    .teachers()                    .get(courseId=c["id"], userId=c["ownerId"])                    .execute()                
    ...
    async def updatemat(self,ctx):
        ...
        async with ctx.typing():
            ...
            for c in courses:
                ...
                cm.append(self.service.courses().courseWorkMaterials().list(courseId=c["id"]).execute())
    ...
    async def update_courses(self):
        ...
        for v1,c1,w1,t1,i in zip(v[:11],c[:11],w[:11],t[:11],range(11)):
            ...
            new=self.service.courses().courseWorkMaterials().list(courseId=c1["id"],pageSize=5).execute()
            ...
            if 'courseWorkMaterial' in new.keys():
             ...
             for i in new['courseWorkMaterial']:
                ...
                if i['id']!=vv1['id']:
                    ...
                    emb=discord.Embed(title=f"{t1['profile']['name']['fullName']} : Updated a material")
                else:
                    break
                #emp.add_field(name="Course",value=f"{c1['name']}  {c1['section']}")
                emb.add_field(name="Course",value=f"{c1['name']}  {c1['section']}")
                emb.add_field(name="Link to the original post",value=f"[Original post link]({vv1['alternateLink']})")
                emb.add_field(name="Description",value=(vv1["description"]if "description"in vv1.keys()else "No description"))
                if 'title' in vv1.keys():
                    emb.add_field(name="Title",value=f"{i['title']}")
                d1=datetime.strptime(
                    vv1["creationTime"].split('.')[0],"%Y-%m-%dT%H:%M:%S",
                    )+timedelta(hours=5,minutes=30)
                d1=d1.strftime("%d-%m-%Y  %a %H:%M")
                    
                d2=datetime.strptime(vv1["updateTime"].split('.')[0],"%Y-%m-%dT%H:%M:%S",
                )+timedelta(hours=5,minutes=30)
                ...
                if "materials" in vv1.keys():
                    ...
                    for j in vv1["materials"]:
                        ...
                        #await ctx.send(f"{j}")
                        if l[0]=="link":
                            emb1.add_field(name="LINK",value=f"[{j['link']['title']}]({j['link']['url']})")
                            
                            
                        elif l[0]=="driveFile":
                            emb1.add_field(name="DRIVEFILE",value=f"[{j['driveFile']['driveFile']['title']}]({j['driveFile']['driveFile']['alternateLink']})")
                            
                        elif l[0]=="youtubeVideo":
                            emb1.add_field(name=f"YOUTUBEVEDIO",value=f"[{j['youtubeVideo']['title']}]({j['youtubeVideo']['alternateLink']})")
                        
                        elif l[0]=="form":
                            emb1.add_field(name='FORM',value=f"[j['form']['title']]({j['form']['formUrl']})")
            ...
            if 'courseWork' in new.keys():
             ...
             for i in new['courseWork']:
                ...
                if i['id']!=vv1['id']:
                    emb=discord.Embed(title=f"{t1['profile']['name']['fullName']} : Posted new Assignment",color=discord.Color.random())
                elif i['id']==vv1['id']and i['updateTime']!=vv1['updateTime']:
                    emb=discord.Embed(title=f"{t1['profile']['name']['fullName']} : Updated a Assignment",color=discord.Color.random())
                else:
                    break
                #emp.add_field(name="Course",value=f"{c1['name']}  {c1['section']}")
                emb.add_field(name="Course",value=f"{c1['name']}  {c1['section']}")
                ...
                if 'dueDate'in vv1.keys():
                    d1=datetime(year=vv1['dueDate']['year'],month=vv1['dueDate']['month'],day=vv1['dueDate']['day'],hour=vv1['dueTime']['hours'],minute=vv1['dueTime']['minutes']
                    )+timedelta(hours=5,minutes=30)
                    d1=d1.strftime("%d-%m-%Y  %a %H:%M")
                else:d1="No due date"
                d2=datetime.strptime(vv1["updateTime"].split('.')[0],"%Y-%m-%dT%H:%M:%S")+timedelta(hours=5,minutes=30)
                d2=d2.strftime("%d-%m-%Y  %a %H:%M")
                ...
                if "materials" in vv1.keys():
                    ...
                    if "updateTime"in l:
                        emb1.add_field(name="Last updated",value=f"{j['updateTime']}")#waste code ig
    ...
    async def materials(self,ctx, coursecode: str):
        ...
        if coursecode in self.cscode:
            async with ctx.typing():
                ...
                i = self.cscode.index(coursecode)
                course = courses[i]
                materials = await self.config.custom("CustomGuildGroup", ctx.guild.id).coursematerials()
                courseworks = await self.config.custom("CustomGuildGroup", ctx.guild.id).coursework()
                material=materials[i]
                a=1
                for c in material["courseWorkMaterial"]:
                    color=discord.Color.random()
                    if "materials" in c.keys():
                        d1=datetime.strptime(c["creationTime"].split('.')[0],"%Y-%m-%dT%H:%M:%S").replace(tzinfo=timezone("Asia/Kolkata")).strftime("%d-%m-%Y  %a %H:%M")
                        d2=datetime.strptime(c["updateTime"].split('.')[0],"%Y-%m-%dT%H:%M:%S").replace(tzinfo=timezone("Asia/Kolkata")).strftime("%d-%m-%Y  %a %H:%M")

                        for j in c["materials"]:
                            emb=discord.Embed(title=f"{c['title']}",color=color)
                            emb.add_field(name="Link to the original post",value=f"[Original post link]({c['alternateLink']})")
                            emb.add_field(name="Description",value=(c["description"]if "description"in c.keys()else "No description"))

                            l=list(j.keys())
                            #await ctx.send(f"{j}")
                            if l[0]=="link":
                                emb.add_field(name=j["link"]["title"],value=f"[link to Website]({j['link']['url']})")
                                
                                
                            elif l[0]=="driveFile":
                                emb.add_field(name=j["driveFile"]["driveFile"]["title"],value=f"[link to driveFile]({j['driveFile']['driveFile']['alternateLink']})")
                                
                            elif l[0]=="youtubeVedio":
                                emb.add_field(name=f"Youtube vedio: {j['youtubeVedio']['title']}",value=f"[link]({j['youtubeVedio']['alternateLink']})")
